...
$totalItemCnt = $collection->count($this->filter);
$metas = $collection->find($this->filter, [ 'limit' => 50, 'skip' => $next * 50 ]);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="p-3">
    <div class="row row-cols-1 row-cols-md-4 row-cols-lg-4 row-cols-xl-6 g-4">
        ...
        <div class="col">
          <div class="card">
            <img src="<?= BASEURL ?>/iiif/<?= $meta['id'] ?>_p0/square/500,/0/default.jpg" class="card-img-top" alt="First page of <?= htmlentities($meta['title']) ?>">
            <div class="card-body">
              <h5 class="card-title"><a href="/artworks/<?= $meta['id'] ?>"><?= htmlentities($meta['title']) ?></a></h5>
            </div>
          </div>
        </div>
        <?php endforeach; ?>
